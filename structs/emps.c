...
int peut_emp(adherent *adh)
{
	if (adh->retard != 0)
	{
		printf("cet adherant est retardere est il ne peut pas avoir une emprunte avant de r�gulariser sa situation");
		return -1;
	}
...
int loader_emp()
{
	...
	while (fgets(buffer, sizeof(buffer), f))
	{
		...
		if (emp->returned == 0)
		{
			...
			else if (emp->delaiLimite->dd < current->dd && emp->delaiLimite->mm == current->mm && emp->delaiLimite->yy == current->yy)
			{
